...
### 10 因子評分模型（v3.3 更新，B7 整合）

| 因子 | 權重 | 計算方式 | 設計理由 |
|------|------|---------|---------|
| **5日動能** | 12% | (今日收盤 / 5日前收盤 - 1) 百分位排名 | 抓短期強勢 |
| **20日動能** | 12% | (今日收盤 / 20日前收盤 - 1) 百分位排名 | 確認中期趨勢 |
| **價值 (Value)** | 13% | P/E 百分位排名（越低越好，反轉） | 找被低估的 |
| **淨值比 (PB Value)** | 8% | P/B 百分位排名（越低越好，反轉） | 強化價值篩選 |
| **品質 (Quality)** | 13% | 殖利率百分位排名（越高越好） | 找獲利穩定的 |
...
| **KD (v3.3 新增)** | 10% | 最新 K 值絕對評分（K < 20 → 95；K > 80 → 10） | 動能超賣加分 |
