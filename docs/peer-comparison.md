...
> v7.3：**移除 `close_to_ma200_ratio` 特徵 + 關閉機率校準** — 診斷發現 ML 模型把 `close_to_ma200_ratio` 學成「ratio 越大、機率越高」的單調關係，Top 20 全部集中在 ratio +24~+144%（已大漲股），與設計理念「波動壓縮起漲股」完全相反。移除該特徵（22 → 21 個），回歸均線糾結 / BB squeeze / 量能突破 等原始起漲訊號為主。同時關閉 Isotonic Calibrator（`ENABLE_CALIBRATION = False`），UI 直接顯示模型原始輸出機率，定義為「10 個交易日內絕對漲幅觸及 +10%（同時跌幅未先觸及 -5%）的機率」。
...
3. **潛力股篩選**：從全台灣上市櫃 ~1,970 支股票中，用 **14 因子**（技術面 50% + 籌碼面 35% + 基本面 15%）百分位排名選出 Top 20
...
# 包含：P/E (20%) + ROE (20%) + 營收趨勢 (25%) + Margin (15%) + 殖利率 (20%)
```

> 注意：這裡不計算技術面分數，因為比較表中沒有足夠的資料（RSI/MACD 等）。
...
| **籌碼面 35%** | trust_net_5d | 12% | 近 5 日投信淨買超張數加總 | 越多越好 | 投信認養（主動型基金拉抬）是中小型波段主力 |
| | inst_volume_ratio | 7% | (\|外資5d\|+\|投信5d\|) / (avg_vol_5d×5) × 100 | 越大越好 | 法人控盤而非散戶當沖跟風 |
| | foreign_net_5d | 6% | 近 5 日外資淨買超張數加總 | 越多越好 | 外資為輔助，避免權值股干擾波段篩選 |
| | trust_streak | 6% | 投信連續淨買天數（負值=連賣） | 越大越好 | 連買 3~5 天以上代表經理人建倉決心 |
...
| **籌碼面** | ❌ | ✅ 投信/外資淨買超, 法人吃貨比, 投信/外資連買天數 |
| **前置過濾** | ❌ | ❌（v7.2 移除 MA200 過濾；v7.3 移除 ratio 特徵，避免模型追高） |
