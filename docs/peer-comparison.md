...
| 因子 | 權重 | 計算方式 | 設計理由 |
|------|------|---------|---------|
| **5日動能** | 15% | (今日收盤 / 5日前收盤 - 1) 百分位排名 | 抓短期強勢 |
| **20日動能** | 15% | (今日收盤 / 20日前收盤 - 1) 百分位排名 | 確認中期趨勢 |
| **價值 (Value)** | 15% | P/E 百分位排名（越低越好，反轉） | 找被低估的 |
| **淨值比 (PB Value)** | 10% | P/B 百分位排名（越低越好，反轉） | 強化價值篩選 |
| **品質 (Quality)** | 15% | 殖利率百分位排名（越高越好） | 找獲利穩定的 |
| **量能比 (Volume Ratio)** | 10% | 今日量 / 近 20 日均量 百分位 | 放量代表關注度 |
| **均線多頭 (MA Trend)** | 10% | 收盤價 > MA5 > MA20 → 100；反之 → 0 | 順勢操作 |
| **低波動 (Low Vol)** | 10% | 近 20 日漲跌幅標準差（越低越好，反轉） | 偏好穩定 |
