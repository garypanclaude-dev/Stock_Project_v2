...
### 11 因子評分模型（v3.4 更新，B8 整合）

| 因子 | 權重 | 計算方式 | 設計理由 |
|------|------|---------|---------|
| **5日動能** | 11% | (今日收盤 / 5日前收盤 - 1) 百分位排名 | 抓短期強勢 |
| **20日動能** | 11% | (今日收盤 / 20日前收盤 - 1) 百分位排名 | 確認中期趨勢 |
| **價值 (Value)** | 12% | P/E 百分位排名（越低越好，反轉） | 找被低估的 |
| **淨值比 (PB Value)** | 8% | P/B 百分位排名（越低越好，反轉） | 強化價值篩選 |
| **品質 (Quality)** | 12% | 殖利率百分位排名（越高越好） | 找獲利穩定的 |
| **量能比 (Volume Ratio)** | 8% | 今日量 / 近 20 日均量 百分位 | 放量代表關注度 |
| **均線排列 (MA Trend, v3.3 升級)** | 8% | 4 態分類（多頭 100 / 空頭 0 / 糾結 50 / 中性 50） | 順勢操作 |
| **低波動 (Low Vol)** | 8% | 近 20 日漲跌幅標準差（越低越好，反轉） | 偏好穩定 |
| **KD (v3.3 新增)** | 9% | 最新 K 值絕對評分（K < 20 → 95；K > 80 → 10） | 動能超賣加分 |
| **OBV (v3.3 新增)** | 8% | 5 日 OBV 斜率 vs 5 日股價斜率比對 | 量價配合度 |
| **K 線型態 (Pattern, v3.4 新增)** | 5% | 近 10 日看多型態計數，看空覆蓋為 30 | 短期反轉訊號 |
