...
> ⚠️ **已棄用 (Deprecated, 2026-06-29)**：此功能已從 `/api/stock-insights` 與前端下線，相關程式碼 (`stock_fetcher/risk.py`、`stock_fetcher/risk_config.py`) 保留但不再被呼叫。本文件僅供歷史參考。

> 版本：1.3
> 最後更新：2026-06-29
> v1.1：新增「警示燈號」(B7 整合) — KD 高檔死叉 / OBV 看空背離 / 均線空頭排列
> v1.2：警示燈號加第 4 類「近期看空反轉型態」(B8 整合)
> v1.3：功能棄用標註（程式碼保留、API 與前端已移除）
